...
pub fn disconnect_event(client: Client<Protocol, Channels>) {
    log::info!("Client disconnected from: {}", client.server_address());
}
...
pub fn insert_component_event(
    mut _event_reader: EventReader<InsertComponentEvent<ProtocolKind>>,
    mut _local: Commands,
...
pub fn update_component_event(
    ...
    if let Some(owned_entity) = &global.owned_entity {
        ...
        let _client_entity = owned_entity.predicted;
        ...
        if let Some(_server_tick) = latest_tick {
            /*
...
pub fn receive_message_event(
    ...
    for event in event_reader.iter() {
        if let MessageEvent(Channels::EntityAssignment, Protocol::EntityAssignment(message)) = event
        {
            ...
            if assign {
                log::info!("gave ownership of entity");
                ...
                if disowned {
                    log::info!("removed ownership of entity");
